#!/usr/bin/env python3
"""
hi-pay-hub/scripts/search-lever.py
Lever job board scraper — Hawaii edition.
Hawaii HRS §378-2.3 (HB 2024, eff. Jan 1, 2024) — employers with 50+ employees must include pay range in all job postings
Run: python3 ~/hi-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/hi-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/hi-pay-hub/scripts/.lever.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = ['houzz', 'hawaiian-airlines']
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))

DISCOVERY_QUERIES = ['site:jobs.lever.co "Hawaii" OR "Honolulu" salary 2025 2026', 'site:jobs.lever.co "Honolulu, HI" OR "Maui" engineer manager analyst']
...
def main():
    ...
    log(f"\n=== Lever scraper complete: {total_found} new HI jobs (api_failures={api_failures}) ===")
    return 0
