#!/usr/bin/env python3
"""
hi-pay-hub/scripts/_common.py
Shared utilities for Hawaii Pay Hub search scripts.
Hawaii HRS §378-2.3 (HB 2024, eff. Jan 1, 2024) — employers with 50+ employees must include pay range in all job postings
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"hi-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/hi-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "HI"
...
REGION_TERMS = ['hawaii', 'honolulu', 'maui', 'kauai', 'hilo', 'oahu', 'kailua', 'kaneohe', 'pearl city', 'waipahu', 'mililani', 'lahaina', 'wailuku', 'kahului', ', hi,', 'hi 96']

_NON_REGION_LOC_TERMS = ['portland, or', 'eugene, or', 'las vegas, nv', 'reno, nv', 'nevada', 'washington, dc', 'washington dc', 'seattle, wa', 'bellevue, wa', 'denver, co', 'boulder, co', 'chicago, il', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'boston, ma', 'minneapolis, mn', 'toronto', 'vancouver', 'montreal']
...
def load_existing_keys():
    ...
    if not shared.exists():
        return existing
    for f in sorted(shared.glob(f"hi-jobs-raw-*.txt")):
        try:
            for line in open(f):
                line = line.strip()
                if not line:
