#!/usr/bin/env python3
"""
hi-pay-hub/scripts/search-workday.py
Workday API scraper — Hawaii edition.

Hawaii Pay Transparency Act (effective Jan 1, 2024)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Hawaii employers
  2. CXS JSON API   — paginate all jobs per employer, filter Hawaii (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/hi-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/hi-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/hi-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Hawaii"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("hawaiianairlines.wd5.myworkdayjobs.com", "hawaiianairlines", "HAEXTCareers", "Hawaiian Airlines"),
    ("kaiserpermanente.wd5.myworkdayjobs.com", "kaiserpermanente", "KPCareer", "Kaiser Permanente"),
    ("walmart.wd5.myworkdayjobs.com", "walmart", "WalmartExternal", "Walmart"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Hawaii" salary 2026',
    'site:myworkdayjobs.com "Honolulu" salary "$" 2026',
    'site:myworkdayjobs.com "Hawaii" hospitality OR healthcare OR tech salary 2026',
    'site:myworkdayjobs.com "Maui" OR "Oahu" OR "Kauai" salary range 2026',
]
...
def is_hawaii(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Hawaii.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_hawaii(p.get("locationsText", ""), p.get("externalPath", "")):
                    hi_jobs.append(p)
            offset += limit
        ...
        log(f"  Hawaii jobs: {len(hi_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(hi_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_hawaii(location, external_path):
            continue
