#!/usr/bin/env python3
"""
hi-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Hawaii edition.
Hawaii HRS §378-2.3 (HB 2024, eff. Jan 1, 2024) — employers with 50+ employees must include pay range in all job postings
Run: python3 ~/hi-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/hi-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/hi-pay-hub/scripts/.greenhouse.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"

log = make_logger(LOG_FILE)
fetcher = Fetcher()

SEED_SLUGS = [('alphahawaii', 'Alpha Inc.'), ('houzz', None), ('hawaiian-airlines', 'Hawaiian Airlines'), ('firsthawaiian', 'First Hawaiian Bank'), ('bankofhawaii', 'Bank of Hawaii'), ('hph', 'Hawaii Pacific Health'), ('kapiolani', 'Kapiolani Medical Center'), ('straub', 'Straub Medical Center'), ('punchbowl', 'Punchbowl'), ('evolve', 'Evolve Vacation Rental')]
...
def main():
    ...
    log("=== HI Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new HI jobs written to {OUTPUT_FILE} ===")
